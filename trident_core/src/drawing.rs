...
use glium::{implement_vertex, DrawError, IndexBuffer, Program, Surface, VertexBuffer};
...
) -> Result<(VertexBuffer<MyVertex>, IndexBuffer<u32>), Box<dyn Error>> {
...
pub fn context_setup(display: &Display<WindowSurface>) -> DrawData {
    let (vertex_buffer, index_buffer) = prepare_drawing(&display).unwrap_or_else(|error| {
        let error_deref = error.deref();
        eprintln!("DRAW PREPARATION FAILED: {error_deref}");
        process::exit(1)
    });

    let vertex_shader = easy_file_io::basic_file_load("map_editor/shaders/vertex.glsl")
        .unwrap_or_else(|e| {
    ...
    let pixel_shader = easy_file_io::basic_file_load("map_editor/shaders/pixel.glsl")
        .unwrap_or_else(|e| {
            let error_deref = e.deref();
            eprintln!("Error occurred in loading shader file: {error_deref}");
    ...
    let program =
        match Program::from_source(display, vertex_shader.as_str(), pixel_shader.as_str(), None) {
            Ok(p) => p,
            Err(e) => {
